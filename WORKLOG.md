...
should merge separately. One thing to reconcile at merge time: whichever
branch lands second should make sure the manager Orders tab (from the other
branch) reads `order.bill` directly instead of recomputing it, now that it's
persisted — that recompute-from-current-% is exactly the bug this session
fixed elsewhere.
