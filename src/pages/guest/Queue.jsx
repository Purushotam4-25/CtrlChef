...
export default function Queue() {
  ...
  async function checkIn() {
    ...
    try {
      ...
      setCheckInError(e.message || "Couldn't check you in — try again.");
    } finally {
      setSubmitting(false);
    }
  }

  const matchLabel = estimateError
    ? "Couldn't check table availability — try again in a moment."
    : !estimate
          ...
          <div className="rounded-xl border p-5" style={{ borderColor: T.border, background: T.panel }}>
            {checkedIn ? (
              ...
              <>
                ...
                />
                <div className="mb-1.5 text-[13px] font-semibold" id="party-size-label">Party size</div>
                <div className="mb-3.5 grid grid-cols-4 gap-1.5 sm:grid-cols-8" role="group" aria-labelledby="party-size-label">
                  {PARTY_SIZES.map((n) => (
                    <button
                      key={n}
                ...
                {checkInError && (
                  <div className="mb-4 rounded-md border border-red-800 bg-red-950/20 px-3 py-2.5 text-[13px] text-red-400">
                    {checkInError}
                  </div>
                )}
                <button
